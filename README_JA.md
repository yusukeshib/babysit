...
ローカルコマンドを透過的な PTY でラップするツール。外部の AI エージェント
（Claude Code, Codex, …）が `gcloud` や `kubectl` のログを読むのと同じ
感覚で、ローカル実行のライブ出力を読めるようにする。
...
TUI も alt-screen もキー横取りもない。出力はそのままターミナルに流れ、
スクロールバックにも残る。Ctrl-C, Ctrl-Z, Ctrl-D など全てのキー入力は
そのままラップ対象のコマンドに届くので、直接コマンドを実行したのと
区別がつかない。babysit の終了コードはラップ対象のコマンドのものを
そのまま返すし、「babysit を終了する」には単にラップされたコマンドを
終わらせればよい（Ctrl-C, `exit` など）。babysit が追加するのは先頭に
表示されるセッション ID だけ。

## インストール

[GitHub Releases](https://github.com/yusukeshib/babysit/releases)
からビルド済みバイナリ（macOS / Linux）を取ってくるか、ソースから
インストールする:
...
output.log      # ラップ対象コマンドの PTY からの生バイト列
